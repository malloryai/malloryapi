...
class Dashboards(SyncResource):
    ...
    def current_events(
        ...
        **kwargs: Any,
        ...
        data = self._http.get(f"{self._path}/current-events", params=params)
        return _parse_paginated(data)

    def vulnerabilities(self, *, limit: int = 24, **kwargs: Any) -> dict[str, Any]:
        params = {"limit": limit, **kwargs}
        return self._http.get(f"{self._path}/vulnerabilities", params=params)

    def latest(self, report_type: str) -> dict[str, Any]:
        return self._http.get(f"{self._path}/{quote(report_type, safe='')}/latest")
...
class AsyncDashboards(AsyncResource):
    ...
    async def current_events(
        ...
        **kwargs: Any,
        ...
        params = {k: v for k, v in params.items() if v is not None}
        data = await self._http.get(f"{self._path}/current-events", params=params)
        return _parse_paginated(data)
    ...
    async def vulnerabilities(
        ...
        params = {"limit": limit, **kwargs}
        return await self._http.get(f"{self._path}/vulnerabilities", params=params)

    async def latest(self, report_type: str) -> dict[str, Any]:
        return await self._http.get(
            f"{self._path}/{quote(report_type, safe='')}/latest"
        )
